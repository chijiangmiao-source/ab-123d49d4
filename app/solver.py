...
def solve(initial_value: int, operations: Sequence[Operation]) -> SolveResult:
    ...
    def dfs(remaining: int, value_idx: int) -> None:
        nonlocal found
        if remaining == 0:
            found += 1
            if not first:
        ...
        for i in index_order:
            ...
            if not remaining & bit or pred[i] & remaining:
                continue  # already placed, or a predecessor is still pending
            kind, a, b = transitions[i]
            if kind == KIND_WRITE:
                nxt = a
            elif kind == KIND_READ:
                if value_idx != a:
                    continue  # read would observe a different value
                nxt = value_idx
            elif value_idx == a:
                if operations[i].success is not True:
                    continue  # matching expected value: CAS must succeed
                nxt = b
            else:
                if operations[i].success is not False:
                    continue  # mismatching value: CAS must fail
                nxt = value_idx
            path.append(i)
            before = found
            dfs(remaining ^ bit, nxt)
            path.pop()
            if found > before:
                feasible_child = True
            if found >= 2:
